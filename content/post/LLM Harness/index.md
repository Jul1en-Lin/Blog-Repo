...
title = "LLM Harness"
slug = "LLM-Harness"
date = "2026-08-04T23:58:53+08:00"
lastmod = "2026-08-04T23:58:53+08:00"
draft = false
description = "阅读 Agent Harness Engineering: A Survey，整理从 Prompt、Context 到 Harness 的演进，以及 ETCLOVG 七层分类、跨层权衡和开放问题。"
categories = ["AI"]
tags = ["AI", "LLM", "Agent", "Harness"]
obsidian_path = "/AI/LLM-Harness"
+++
# LLM Harness

最近看到了一篇关于 [Agent Harness Engineering: A Survey](https://picrew.github.io/LLM-Harness/) 的论文，在这里做个学习记录。

## 从 Prompt 到 Harness

工程的边界一直在扩大：Prompt Engineering → Context Engineering → Harness Engineering。每个阶段关注的问题不一样，但都在摸索围绕模型的工程该长什么样。

![Prompt、Context 与 Harness 的关系](assets/prompt-context-harness.png)
...
![Harness 时间线](assets/harness-timeline.png)
...
![ETCLOVG 七层分类](assets/etclovg-seven-layers.png)

### 详细分类视图

![ETCLOVG 详细分类视图](assets/etclovg-detail.png)
...
**C 上下文与记忆管理：** 叶子按时间跨度排开：短期活跃窗口、中期会话状态与跨次运行的持久化、长期记忆系统，再加上长程上下文技术和 context drift 的边界问题。[RAG]({{< relref "/post/概念/" >}}) 这类检索增强技术也落在这一层。
...
**G 治理与安全：** 权限模型与身份管理、生命周期 [Hooks]({{< relref "/post/概念/" >}})、组件加固、声明式 constitutions、审计基础设施，最后是对整个 agent 安全威胁面的梳理。
...
**Harnesses 是独立系统层**
真实场景中的可靠性很大一部分取决于执行控制、反馈循环、治理、评估和运维设计，对模型本身能力的要求反而没有那么直接。很多时候瓶颈在 harness，未来的发展关注点也会从模型能力延伸到 Harness 系统。
...
论文给出的结论是：**harness 的改动应该按系统级改动来测试**，不能只在单个模块上做优化，再假设其他层不受影响。一个局部改动可能改变模型的执行过程，最终产生错误结果 =v=
...
这个转变的核心驱动是：**单人实验时，框架够用；多人长期用时，需要平台**。未来 1-2 年里，harness 工程会从"搭框架"走向"搭平台"，把可观测层、验证层、治理层这类**插件**逐渐变成**核心功能**。
...
- 未来 traces 应该成为主要对象，可以从中直接计算输出权重、失败归因和回归测试结果
...
**1. Harness 是独立的系统层**
现在大家都会关注模型评分，判断模型有多强、跑分有多高，也容易认为 agent 的上限由模型决定。但这篇论文把视角放到了另一边：生产环境的下限由 harness 决定。模型决定任务难度的上限，harness 决定系统能不能持续、可复现地把任务做完，这个视角把软件工程问题放到了和模型能力同等的位置。这是我觉得 harness 可以作为独立系统层的依据 =v=
...
**3. 自适应简化**
五个开放问题里我最感兴趣的是这一个。未来的 harness 会不会变成一个能够自动优化自己的系统？这和模型量化有点像，但针对的是工程层。如果这个方向有突破，harness 工程可能会从手工强化编排转向根据模型能力进行自动优化。

**4. 从框架到平台的转变**
这个趋势其实已经在发生了，Cursor、Hermes 这些产品本质上是把 LangChain 级别的能力包装成了持久化产品。如果这个转变加速，未来 harness 工程的门槛会降低，但深度定制的门槛会提高。
![从框架到平台的转变](assets/harness-platform-shift.png)
