...
The current header brand and browser favicon now use the same `JL` signature path, with the header rendering as a theme-colored stroke-dash handwriting animation and browser icons rendering as a white signature on black.
